    ...
    <div className="flex min-h-screen flex-col bg-slate-900 text-slate-50">
      ...
      <main className="flex-1 space-y-4 px-4 pb-6 pt-2">
        <section className="rounded-2xl bg-slate-800/80 p-4">
          <div className="mb-3 flex flex-col gap-2 text-xs sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <select
                ...
                className="rounded-lg border border-slate-600 bg-slate-900 px-2 py-1 text-xs text-slate-50"
              >
              ...
              <input
                ...
                className="w-20 rounded-lg border border-slate-600 bg-slate-900 px-2 py-1 text-xs text-slate-50"
              />
              <button
                type="button"
                onClick={applyFilter}
          ...
          {loading ? (
            <p className="text-xs text-slate-300">Memuat jadwal...</p>
          ) : error ? (
            <p className="text-xs text-red-400">{error}</p>
          ) : daysWithShift.length === 0 ? (
            <p className="text-xs text-slate-300">
              Belum ada jadwal shift pada bulan ini.
            </p>
          ) : (
              ...
              {daysWithShift.map((item) => {
                ...
                return (
                  <div
                    ...
                    className="rounded-xl bg-slate-900/70 p-3 text-[11px]"
                  >
